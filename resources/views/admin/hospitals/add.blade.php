    ...
    <section class="content">
        <div class="container-fluid">
            <div class="card card-default">
                <form action="{{ isset($hospital) ? route('hospital.update', $hospital->id) : route('hospital.store') }}" method="POST" name="HospitalCreateUpdate" enctype="multipart/form-data">
                    @csrf
                    @isset ($hospital)
                        @method('PUT')
                    @endisset
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label><strong>User <span class="text-danger">*</span></strong></label>
                                    <select class="form-control" name="user_id" >
                                        @foreach( $users as $user )
                                            <option value="{{ $user->id }}">{{ $user->name }}</option>
                                        @endforeach
                                    </select>
                                    <span class="text-danger"><b>{{ $errors->first('user_id') }}</b></span>
                                </div>
